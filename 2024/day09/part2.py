...
while file_block:
    ...
    if free_block and free_index < file_index:
        ...
        if free_block.size == file_block.size:
            ...
            blocks.insert(file_index, Block(-1, file_block.size))
        elif free_block.size > file_block.size:
            blocks[free_index] = file_block
            blocks.insert(free_index + 1, Block(-1, free_block.size - file_block.size))
            blocks.insert(file_index + 1, Block(-1, file_block.size))
